...
try:
    from data_checker_v2 import DataChecker
    _HAS_DATA_CHECKER = True
except ImportError:
    _HAS_DATA_CHECKER = False
...
class ClusterViewer(QMainWindow):
    def __init__(self, duplicate_pairs, crop_data=None, embedding_mode="option1"):
        ...
        self.btn_move_left = QPushButton("🗑 Remove Left")
        self.btn_move_left.setObjectName("Danger")
        self.btn_move_left.clicked.connect(lambda: self.move_to_trash("left"))

        self.btn_move_right = QPushButton("🗑 Remove Right")
        self.btn_move_right.setObjectName("Danger")
        self.btn_move_right.clicked.connect(lambda: self.move_to_trash("right"))
        ...
        controls_layout.addWidget(self.btn_move_left)
        controls_layout.addWidget(self.btn_move_right)
    ...
    def move_to_trash(self, side="right"):
        if not self.pairs:
            return

        path1, path2, score = self.pairs[self.idx]
        target = path1 if side == "left" else path2
        ...
        if reply == QMessageBox.Yes:
            self._process_file_action(target, "Moved")
...
class YOLOInspector(QMainWindow):
    def __init__(self):
        ...
        btn_data_checker = QPushButton("🔍 Open Data Checker")
        btn_data_checker.setStyleSheet("background-color: #0891b2; color: white; border: 1px solid #0e7490; border-radius: 6px; font-size: 13px; padding: 8px 20px; font-weight: bold;")
        btn_data_checker.setEnabled(_HAS_DATA_CHECKER)
        btn_data_checker.setToolTip("" if _HAS_DATA_CHECKER else "data_checker.py not found")
        btn_data_checker.clicked.connect(self.open_data_checker)
        self.layout.addWidget(btn_data_checker)
        self._data_checker_win = None
    ...
    def open_data_checker(self):
        if self._data_checker_win and self._data_checker_win.isVisible():
            self._data_checker_win.activateWindow()
            return
        self._data_checker_win = DataChecker()
        self._data_checker_win.show()
