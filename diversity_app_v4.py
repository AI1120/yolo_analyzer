...
class EmbeddingWorker(QThread):
    ...
    def get_image_paths_from_yaml(self, yaml_path):
        ...
        # REMOVED: Old max_images limit (no longer needed)
        ...
        # NEW: Apply random sampling if enabled
    ...
    def extract_yolo_features(self):
        ...
        # NOW receives 3 values: image_data, image_dir, class_names
        ...
        for i, (img_path, split_label) in enumerate(tqdm(image_data)):
            try:
                if hasattr(yolo_model, 'model') and len(yolo_model.model) > 0:
                    ...
                    # NEW: Get class_id from label file instead of split_label
        ...
        # NEW: Return 5 values (coords, paths, class_ids, image_dir, class_names)
    ...
    def extract_clip_features(self):
        ...
        for i, (img_path, split_label) in enumerate(tqdm(image_data)):
            try:
                # NEW: Get class_id from label file
                class_id = self.get_class_id_from_label(img_path)
                if class_id == 3:
                    print('\n', class_id, img_path)
                ...
                batch_class_ids.append(class_id)  # Store class_id instead of split
...
class DiversityApp(QMainWindow):
    def __init__(self):
        ...
        self.split_colors = []  # Will be populated dynamically
        self.split_names = []   # Will be class names from YAML
    ...
    def init_ui(self):
        ...
        self.viz_method_combo.addItems(["t-SNE", "UMAP"])
        ...
        # sampling_layout.addWidget(QLabel("Sample Size: "))
    ...
    def start_analysis(self):
        ...
        # NEW: Get sampling parameters from UI
        ...
        self.worker = EmbeddingWorker(mode, yolo_weights, yolo_model, 
                                    self.yaml_file_path, clip_model, 
    ...
    def draw_plot(self):
        ...
        # Convert class_ids to position indices for coloring
        split_indices = np.array([self.class_id_to_index.get(int(c), 0) for c in splits])
        
        self.scatter_plot = ax.scatter(
            self.tsne_coordinates[:, 0], 
            self.tsne_coordinates[:, 1], 
            s=60, alpha=0.7, c=split_indices,  # Use position indices, not class_ids
    ...
    def display_current_image(self):
        ...
        class_id = int(self.image_splits[self.current_image_index])
        class_name = self.class_names_map.get(class_id, f"Class {class_id}")
        self.selected_image_info.setText(f"{os.path.basename(img_path)} | {class_name}")
